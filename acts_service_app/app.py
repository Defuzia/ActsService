...
@app.route("/admin")
def admin():
    return render_template("admin.html", template_exists=TEMPLATE_PATH.exists())
...
def upload_template():
    ...
    return jsonify(ok=True, message="✅ Шаблон успешно обновлён!")
...
def generate():
    ...
    if not TEMPLATE_PATH.exists():
        return jsonify(ok=False, error="Шаблон uploads/template.docx не найден. Загрузите шаблон через /admin."), 400

    try:
        zip_path = build_archive(acts)
...
def build_archive(acts: list[dict[str, str]]) -> Path:
    OUTPUT_DIR.mkdir(exist_ok=True)
