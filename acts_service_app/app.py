...
def make_act_filename_base(sign_datetime: str, act_number: int, used_base_names: set[str]) -> str:
    ...
    Акт оказанных услуг - {SIGN_DATETIME_SHORT}

    Пример: Акт оказанных услуг - 20260802064213

    Суффикс для уникальности добавляется только если имя уже занято
    (например, две строки с одинаковой датой/временем подписи).
    """
    ...
    base_name = f"Акт оказанных услуг - {sign_datetime_short}"
    if base_name not in used_base_names:
        used_base_names.add(base_name)
        return base_name

    # Имя занято — добавляем порядковый номер для уникальности.
    for i in range(1, 1000):
        candidate = f"Акт оказанных услуг - {sign_datetime_short} ({i})"
...
def create_docx_from_template(template_path: Path, output_path: Path, replacements: dict[str, str]) -> None:
    ...
    # Убираем ВЕРХНИЙ колонтитул, чтобы он не сдвигал содержимое вниз.
    # Нижний пустой колонтитул (в оригинале — Arial 8, слева) оставляем.
    # Важно делать ПОСЛЕ подстановки меток: обращение к section.header
    # в iter_text_xml_roots создаёт колонтитул заново.
    remove_headers(document)
    document.save(output_path)


def remove_headers(document) -> None:  # type: ignore[no-untyped-def]
    """Удаляет ссылки на верхние колонтитулы во всех секциях (нижние оставляет)."""
    from docx.oxml.ns import qn
    ...
    for section in document.sections:
        ...
        for ref in sect_pr.findall(qn("w:headerReference")):
            sect_pr.remove(ref)
        # Убираем признак отдельного колонтитула первой страницы, чтобы Word
        # не показывал/не резервировал место под верхний колонтитул.
