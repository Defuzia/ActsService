...
def generate():
    ...
    try:
        zip_path = build_archive(acts, mode=mode)
...
def build_archive(acts: list[dict[str, str]], mode: str = "act_offer") -> Path:
    OUTPUT_DIR.mkdir(exist_ok=True)
...
def make_act_filename_base(sign_datetime: str, act_number: int, used_base_names: set[str]) -> str:
    ...
    Акт - {SIGN_DATETIME_SHORT}-{ACT_DATE}.{RND2}
    """
    ...
    for _attempt in range(100):
        ...
        base_name = f"Акт - {sign_datetime_short}-{act_date}.{rnd2}"
