const COLUMNS = [
  ...
  { key: 'OFFER_SENT_DATE', label: 'Дата отправки оферты', placeholder: '01.01.2026', picker: 'date', icon: 'calendar', group: 'offer' },
  { key: 'EXEC_PHONE', label: 'Телефон исполнителя', placeholder: '+7 900 000-00-00', icon: 'phone', group: 'offer' },
  { key: 'EXEC_EMAIL', label: 'E-mail исполнителя', placeholder: 'mail@example.com', icon: 'mail', group: 'offer' },
  { key: 'EXEC_ACCBANK', label: 'Банковские реквизиты', placeholder: 'р/с ..., банк ...', icon: 'bank', group: 'offer' },
  { key: 'SIGN_DATETIMEOFF', label: 'Дата подписи оферты', placeholder: '01.01.2026 00:00:00', picker: 'datetime', icon: 'calendar', group: 'offer' },
...
function initIndexPage() {
  ...
  let selectedMode = 'act_offer'; // 'act_offer' | 'act_only' | 'offer_only'
  let progressTimer = null;
  let currentProgress = 0;

  updateEmptyState();

  // Тумблер количества копий.
  ...
  if (copiesInput) {
    ...
    if (copiesMinus) {
      copiesMinus.addEventListener('click', () => {
        copiesInput.value = clampCopies((parseInt(copiesInput.value, 10) || 1) - 1);
      });
    }
    if (copiesPlus) {
    ...
    try {
      const response = await fetch('/generate', {
        ...
        body: JSON.stringify({
          acts,
          mode: selectedMode,
          copies: copiesInput ? clampCopies(copiesInput.value) : 1,
        }),
      });
  ...
  function addRow(container, initialData = {}) {
    ...
    COLUMNS.forEach((column) => {
      if (column.group === 'offer' && !offerDividerAdded) {
