const COLUMNS = [
  ...
  // Поля оферты:
  { key: 'OFFER_SENT_DATE', label: 'Дата отправки оферты', placeholder: '01.01.2026', picker: 'date', icon: 'calendar' },
  { key: 'EXEC_PHONE', label: 'Телефон исполнителя', placeholder: '+7 900 000-00-00', icon: 'phone' },
  { key: 'EXEC_EMAIL', label: 'E-mail исполнителя', placeholder: 'mail@example.com', icon: 'mail' },
  { key: 'EXEC_ACCBANK', label: 'Банковские реквизиты', placeholder: 'р/с ..., банк ...', icon: 'bank' },
  { key: 'SIGN_DATETIMEOFF', label: 'Дата подписи оферты', placeholder: '01.01.2026 00:00:00', picker: 'datetime', icon: 'calendar' },
];
  ...
  generateBtn.addEventListener('click', async () => {
    ...
    try {
      const response = await fetch('/generate', {
        ...
        body: JSON.stringify({ acts, mode: selectedMode }),
      });
