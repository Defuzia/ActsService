...
const TEST_MODE_KEY = 'acts_test_mode';
...
function initIndexPage() {
  ...
  // Разблокировка режимов «Акт + Оферта» и «Только оферта» в тестовом режиме.
  function applyTestMode() {
    const on = isTestModeOn();
    modeOptions.forEach((option) => {
      const isOfferMode = option.dataset.mode === 'act_offer' || option.dataset.mode === 'offer_only';
      if (!isOfferMode) return;
      const lock = option.querySelector('.mode-lock');
      if (on) {
        option.disabled = false;
        option.classList.remove('locked');
        option.removeAttribute('title');
        if (lock) lock.style.display = 'none'; // убираем иконку замка
      } else {
        option.disabled = true;
        option.classList.add('locked');
        option.title = 'Функционал будет доступен позже';
        if (lock) lock.style.display = ''; // возвращаем замок
      }
    });
  }
  applyTestMode();
...
function initAdminPage() {
  ...
  if (testModeBtn) {
    const labelEl = testModeBtn.querySelector('.test-mode-label');

    function refreshTestBtn() {
      const on = isTestModeOn();
      testModeBtn.classList.toggle('active', on);
      if (labelEl) {
        labelEl.textContent = on ? 'Тестовый режим: вкл' : 'Тестовый режим: выкл';
      }
    }

    testModeBtn.addEventListener('click', () => {
      const on = isTestModeOn();
      localStorage.setItem(TEST_MODE_KEY, on ? 'false' : 'true');
      refreshTestBtn();
      const newState = !on;
      alert(newState
        ? 'Тестовый режим включён. На главной странице режимы «Акт + Оферта» и «Только оферта» теперь доступны.'
        : 'Тестовый режим выключен. Режимы «Акт + Оферта» и «Только оферта» снова заблокированы.');
    });
